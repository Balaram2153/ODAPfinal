...
var phone= storedResponse.patientMobileNumber;
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (digits.test(referaldoctorname)) // Check if digits are present
        throw new Error("Enter valid referred doctor name without digits");
      if (!alphabets.test(referaldoctorname)) // Check if alphabets are absent
        throw new Error("Enter valid referred doctor name with at least one alphabet");
      if (splchars.test(referaldoctorname)) // Check if special characters are present
        throw new Error("Enter valid referred doctor name without special characters");
      if (referaldoctorname.length < 5)
        throw new Error("Doctor name should have more than 5 characters");
   ...
   if (attendee==="")
     ...
     if(digits.test(attendee))
     throw new Error("attendee name accepts only characters") 
   if (splchars.test(attendee)) // Check if special characters are present
     throw new Error("Attendee name accepts only characters");
  ...
  return (
    ...
    aria-label=".form-select-sm example"  />
          ...
          <tr>
            <td for='date' >Date :</td>
            <td>            <input type='date' className="form-control" id='Bookingdate' value={bookingdate}  onChange={handleDateChange} placeholder='enter mobile number' required />
            </td>
          </tr>
          <tr>
            <td>slot :</td>
